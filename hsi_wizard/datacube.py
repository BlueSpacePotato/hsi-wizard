...
class DataCube(metaclass=TrackExecutionMeta):
    ...
    def __init__(self, cube=None, wavelengths=None, name=None, notation=None, record: bool = False) -> None:
        ...
        self.shape = None if cube is None else cube.shape  # shape of the dc like np.array().shape
        self.dim = None  # get dimension of the dc 2d, 3d, 4d ...
        self.wavelengths = np.array(wavelengths) if wavelengths is not None \
            else np.arange(0, cube.shape[0], dtype=int) if cube is not None else None
    ...
    def __add__(self, other):
        ...
        if self.wavelengths is None or other.wavelengths is None:
            print('\033[93mOne of the two DataCubes dos not contain the wavelength information, add will work, '
                  'but you lose these information\033[0m')
        else:
            new_wavelengths = self.wavelengths + other.wavelengths

        new_cube = np.concatenate((self.cube, other.cube), axis=0)
        return DataCube(cube=new_cube, wavelengths=new_wavelengths, name=self.name, notation=self.notation)
    ...
    def resize(self, x_new: int, y_new: int, interpolation: str = 'linear'):
        ...
        cv2.INTER_CUBIC 	The bicubic interpolation with 4×4-pixel neighborhoods, which, though slow to run,
                            generates high-quality instances.
        cv2.INTER_LANCZOS4 	The Lanczos interpolation with an 8×8-pixel neighborhood, which generates images
                            of the highest quality but is the slowest to run.
    ...
    # todo: implemnt
    def shift_layers(self, num_layer: int):
        if num_layer > self.shape[0]:
            raise ValueError(f'`Num_layer` {num_layer} must me <= then the layer deeps of the DataCube {self.shape[0]}')
        raise NotImplementedError('Sorry - Not Implemented')

    def set_wavelengths(self, wavelengths: np.array):
        if not isinstance(wavelengths, np.ndarray):
            try:
                # todo: better error handling
                self.wavelengths = np.array(wavelengths)
            except AttributeError:
                raise AttributeError('Your wavelengths didnt match an 1d np.array')

        else:
            self.wavelengths = wavelengths

    def set_cube(self, cube: np.array):
        if not isinstance(cube, np.ndarray):
            try:
                # todo: better error handling
                cube = np.array(cube)
            except AttributeError:
                raise AttributeError('Your cube is not convertable to a np.array')
        if 3 <= cube.ndim <= 4:
            self.cube = cube
        elif cube.ndim == 2:
            self.cube = np.zeros(shape=(1, cube.shape[0], cube.shape[1]), dtype=cube.dtype)
            self.cube[0] = cube
    ...
    def load_template(self, filenmae):
