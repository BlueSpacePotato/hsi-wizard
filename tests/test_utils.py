...
from wizard._utils._loader import pickle, folder, nrrd, tdms, xlsx, csv
...
class TestLoader:
    ...
    def test_read_folder_filter_image_files(self):
        ...
        assert folder.filter_image_files(files) == expected
    ...
    def test_read_folder_load_image(self):
        ...
        try:
            img = folder.load_image(temp_path)
    ...
    def test_read_folder_image_to_dc_single(self):
        ...
        try:
            dc = folder.image_to_dc(temp_path)
            print(dc.shape)
    ...
    def test_read_folder_image_to_dc_batch(self):
        ...
        for _ in range(3):
            ...
            plt.imsave(temp_file.name, np.random.rand(10, 10, 3))
            temp_file.close()

        try:
            dc = folder.image_to_dc(temp_files)
            print(dc)
            assert isinstance(dc, DataCube)
            assert dc.shape[0] == 3  # Batch size should match
        finally:
    ...
    def test_read_folder_no_images(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            with pytest.raises(ValueError, match="No valid image files found in the directory."):
                folder._read_folder(temp_dir)

    def test_read_tdms_invalid_file(self):
        with pytest.raises(FileNotFoundError):
            tdms._read_tdms("non_existent_file.tdms")
    ...
    def test_write_and_read_xlsx(self, sample_data_cube, tmp_path):
        ...
        xlsx._write_xlsx(test_cube, str(test_file))

        # Read back the DataCube
        read_cube = xlsx._read_xlsx(str(test_file))
    ...
    def test_write_and_read_csv(self, sample_data_cube, tmp_path):
        ...
        csv._write_csv(test_cube, str(test_file))

        # Read back the DataCube
        read_cube = csv._read_csv(str(test_file))
