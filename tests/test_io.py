...
def sample_data_cube():
    data = np.random.rand(10, 11, 12)
    wavelengths = [i for i in range(data.shape[0])]
    name = "TestCube"
    notation = "nm"
    record = False
    return wizard.DataCube(cube=data, wavelengths=wavelengths, name=name, notation=notation, record=record)
...
class TestLoaderHelper:
    ...
    def test_valid_directory(self):
        ...
        # Act
        result = io._helper.get_files_by_extension(path, extension)
    ...
    def test_empty_string_input(self):
        ...
        with pytest.raises(ValueError):
            io._helper.make_path_absolute(path)
    ...
    def test_transform_2d_to_3d(self):
        ...
        # Act
        result = io._helper.to_cube(data, len_x, len_y)
...
class TestLoaderNrrd:
    ...
    def test_read_write_nrrd(self, sample_data_cube):
        ...
        try:
            ...
            io.nrrd._write_nrrd(dc=sample_data_cube, path=temp_path)

            # Read the DataCube from the temporary file
            loaded_data_cube = io.nrrd._read_nrrd(path=temp_path)
...
class TestLoaderFolder:
    def test_read_folder_filter_image_files(self):
        ...
        assert io.folder.filter_image_files(files) == expected
    ...
    def test_read_folder_load_image(self):
        ...
        try:
            img = io.folder.load_image(temp_path)
    ...
    def test_read_folder_image_to_dc_single(self):
        ...
        try:
            dc = io.folder.image_to_dc(temp_path)
            print(dc.shape)
            assert isinstance(dc, wizard.DataCube)
    ...
    def test_read_folder_image_to_dc_batch(self):
        ...
        try:
            dc = io.folder.image_to_dc(temp_files)
    ...
    def test_read_folder_image_to_dc_batch_pusbroom(self):
        ...
        try:
            dc = io.folder.image_to_dc(temp_files, type='pushbroom')
    ...
    def test_read_folder_no_images(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            with pytest.raises(ValueError, match="No valid image files found in the directory."):
                io.folder._read_folder(temp_dir)

class TestLoaderTDMS:

    def test_read_tdms_invalid_file(self):
        with pytest.raises(FileNotFoundError):
            io.tdms._read_tdms("non_existent_file.tdms")
...
class TestLoaderXLSX:
    ...
    def test_write_and_read_xlsx(self, sample_data_cube, tmp_path):
        ...
        io.xlsx._write_xlsx(test_cube, str(test_file))
...
class TestLoaderCSV:
    ...
    def test_write_and_read_csv(self, sample_data_cube, tmp_path):
        ...
        io.csv._write_csv(test_cube, str(test_file))
...
class TestLoaderHdr:
    def test_write_and_read_hdr(self, tmp_path):
        ...
        io.hdr._write_hdr(test_cube, str(hdr_path))
